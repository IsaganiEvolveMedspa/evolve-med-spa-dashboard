...
// window ends on the SAME elapsed day-of-month as the current period's data cutoff
// (clamped to the prior month's length) so MTD compares like-for-like — e.g. with
// data through Jul 9 the prior window is Jun 1–9, not all of June. Falls back to the
// full prior month when endDate is omitted.
//
// endDate is the SELECTED month's end (always month-end, e.g. Jul 31), so on its own
// it would make the prior window the full prior month. latestDate is the last day
// that actually has data; for the current (partial) month it lands mid-month (Jul 9)
// and we clamp to it, while for a complete past month it is later than endDate so
// min() keeps endDate — i.e. complete months still compare full-vs-full.
const prevMonthRange = (startDate, endDate, latestDate) => {
  if (!startDate) return { start: undefined, end: undefined };
  const effEnd = (latestDate && endDate && latestDate < endDate) ? latestDate : endDate;
  const d = new Date(startDate);
  const y = d.getFullYear(), m = d.getMonth() - 1;
  const py = m < 0 ? y - 1 : y, pm = (m + 12) % 12;
  if (!effEnd) return { start: firstOf(py, pm), end: lastOf(py, pm) };
  const endDay = Number(String(effEnd).slice(8, 10));    // "YYYY-MM-DD" -> DD
...
// Utilization / rev-per-hr threshold pill colors + backgrounds (from spec legend).
const PILL_RED = '#FBE3E1', PILL_GOLD = '#FBF1D6', PILL_TEAL = '#DDF0E6';
const utilPill = (v) => {
  const x = pctScale(v);
  if (x === null) return { color: C.ink2, bg: 'transparent' };
  if (x < 60) return { color: C.red, bg: PILL_RED };
  if (x < 75) return { color: C.gold, bg: PILL_GOLD };
  return { color: C.teal, bg: PILL_TEAL };
};
const revHrPill = (v, kind) => {
  const x = n(v); if (x === null) return { color: C.ink2, bg: 'transparent' };
  const lo = kind === 'esth' ? 125 : 450;
  const hi = kind === 'esth' ? 175 : 550;
  if (x < lo) return { color: C.red, bg: PILL_RED };
  if (x < hi) return { color: C.gold, bg: PILL_GOLD };
  return { color: C.teal, bg: PILL_TEAL };
};
...
const OverviewView = () => {
  ...
  const prev = prevMonthRange(fl.start_date, fl.end_date, fl.latestDate);
...
const LocationPerformanceTable = ({ rows, range, h, totals }) => {
  ...
  // Cash MTD cell: value + bar vs budget (marker at 100%).
  const cashCell = (l) => {
    const p = paceOf(l.monthly_budget, l.trending);
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 150 }}>
        <span style={{ font: `600 11.5px ${FONT}`, color: C.ink, whiteSpace: 'nowrap' }}>{money(l.cash_sales, { compact: true, floor: true })}</span>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', font: `500 11.5px ${FONT}`, color: C.ink2 }}>
          ...
          <tbody>
            {rows.map((l) => {
              ...
              return (
                <tr key={l.location} className="ev-lrow">
                  ...
                  <td style={{ ...td, textAlign: 'center' }}>{num(o.new_client_count)}</td>
                  <td style={{ ...td, textAlign: 'center' }}>{num(o.existing_client_count)}</td>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={16} style={{ ...td, textAlign: 'center', color: C.gray, padding: '20px 9px' }}>No location data for this range.</td></tr>
            )}
          </tbody>
          {rows.length > 0 && (
            <tfoot>
              <tr>
                ...
                <td style={{ ...totCell, textAlign: 'left' }}>{money(totCash, { compact: true, floor: true })}</td>
