"""
Regenerate data/rebooking_daily.json from the daily Zenoti "Business KPI" exports.

Each source file (data/new_guests/Business KPI <Month> <Day>.csv) is one calendar
day's chain export with per-center rows and a "Rebooking Source %" column — the
official rebooking rate we surface on the dashboard.
...
Output shape (read at runtime with stdlib json only — no pandas/openpyxl in prod):
    { "YYYY-MM-DD": { "Center Name": <rebooking_pct float>, ... }, ... }

Run after dropping new daily files into data/new_guests/:
    python scripts/build_rebooking_json.py
"""
import csv
import json
import os
...
OUT    = os.path.join(HERE, "data", "rebooking_daily.json")

CENTER_COL = "Center Name"
REBOOK_COL = "Rebooking Source %"
...
def build() -> dict[str, dict[str, float]]:
    ...
    for fname in sorted(os.listdir(SRCDIR)):
        ...
        with open(path, newline="", encoding="utf-8-sig") as f:
            ...
            if CENTER_COL not in reader.fieldnames or REBOOK_COL not in reader.fieldnames:
                raise ValueError(f"{fname} missing '{CENTER_COL}'/'{REBOOK_COL}'")
            per_center: dict[str, float] = {}
            for row in reader:
                center = (row.get(CENTER_COL) or "").strip()
                raw    = (row.get(REBOOK_COL) or "").strip()
                if not center:
                    continue
                try:
                    per_center[center] = float(raw or 0)
                except ValueError:
                    per_center[center] = 0.0
        out[iso] = per_center
    return out
