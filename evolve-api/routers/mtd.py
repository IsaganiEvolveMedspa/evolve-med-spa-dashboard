...
def get_mtd_kpi_header(
    ...
    try:
        ...
        yesterday = # "Prior Day Sales" = the most recent day that actually has cash data at or
                    # before end_date (for the selected locations) — NOT end_date - 1. Cash data
                    # lags the calendar, so end_date - 1 returns $0 whenever the range runs ahead
                    # of loaded data. Resolve the real latest data day instead.
