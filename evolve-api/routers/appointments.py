...
def get_appointments_summary(
    ...
    try:
        ...
        SELECT
            ...
            COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) IN ('no show', 'closed no show')      THEN CONCAT(CAST(invoice_no AS VARCHAR(64)), '|', guest_name) END) AS no_shows,
            COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) IN ('cancelled', 'closed cancellation') THEN CONCAT(CAST(invoice_no AS VARCHAR(64)), '|', guest_name) END) AS cancellations,
            COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) = 'deleted'   THEN invoice_no END) AS deleted,
            COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) IN ('no show', 'closed no show') THEN CONCAT(CAST(invoice_no AS VARCHAR(64)), '|', guest_name) END) * 1.0
                / NULLIF(COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) != 'deleted' THEN invoice_no END), 0)
                * 100                                                                               AS no_show_rate,
            COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) IN ('cancelled', 'closed cancellation') THEN CONCAT(CAST(invoice_no AS VARCHAR(64)), '|', guest_name) END) * 1.0
                / NULLIF(COUNT(DISTINCT CASE WHEN add_on = 'No' AND LOWER(status) != 'deleted' THEN invoice_no END), 0)
